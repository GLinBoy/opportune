          ...
          <v-card-text class="d-flex align-center">
            ...
            <div class="flex-grow-1">
              <div class="d-flex align-center mb-1">
                ...
                <v-chip
                  :color="session.status === 'ACTIVE' ? 'success' : session.status === 'REVOKED' ? 'error' : 'grey'"
                  size="x-small"
                  variant="flat"
                >
                  {{ session.status }}
            ...
            <v-btn
              ...
              @click="$emit('terminate', session.refreshTokenId)"
            >
              Revoke
            </v-btn>
          </v-card-text>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    watch(() => [props.sessions, props.highlightedSessionId] as const, () => {
    ...
    return { formatDate, setSessionRef, isCurrentSession }
  }
})
</script>
...
@keyframes highlight-pulse {
  0%, 100% { box-shadow: 0 0 8px rgba(var(--v-theme-warning), 0.3); }
  50% { box-shadow: 0 0 16px rgba(var(--v-theme-warning), 0.6); }
}
</style>
